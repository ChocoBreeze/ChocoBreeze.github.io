...
- JSON-LD는 없습니다.
- `check:content`는 Markdown 내부 링크와 생성된 `/blog/...` 경로를 검사해 누락 대상을 경고합니다.
...
1. JSON-LD에 필요한 값을 기존 Props에서 얻을 수 있는지 확인하고 타입을 확장합니다.
2. JSON 직렬화 시 `<` 등 스크립트 문맥 문자를 안전하게 처리합니다.
3. `BlogPosting`을 먼저 적용하고 빌드 HTML을 검증합니다.
4. 실제 사용자 경로를 반영한 breadcrumb를 추가합니다.
5. 링크 검사를 `확실한 오류`와 `레거시 경고`로 구분합니다.
6. `/blog/...` 누락을 오류로 승격한 뒤 정적 라우트와 앵커 검사를 순차 추가합니다.
