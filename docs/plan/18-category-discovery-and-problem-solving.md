...
- Problem Solving 페이지는 이미 Easy·Medium·Hard 집계와 단일 난이도 필터, 상위 8개 토픽 집계를 제공합니다.
- 토픽 표시는 현재 정보 제공용이며 필터로 동작하지 않습니다.
- 필터와 더 보기 로직은 각각 페이지 스크립트와 공통 레이아웃에 나뉘어 있습니다.
...
1. 카드가 노출할 필터 데이터와 URL 파라미터 이름을 확정합니다.
2. 공통 정렬·더 보기·필터가 하나의 상태에서 동작하도록 로직을 정리합니다.
3. 연도와 태그 필터를 한 카테고리에 파일럿 적용합니다.
4. Problem Solving의 기존 난이도 필터를 공통 상태 모델에 연결합니다.
