...
5. [완료] 상위 토픽 칩을 공통 상태 기반 필터로 확장했습니다.
6. [완료] `platform`·`problemNumber`를 선택 frontmatter와 신규 글 스캐폴드에 추가하고, 제목 파싱 없이 공통 필터에 연결했습니다. 기존 글은 백필하지 않습니다.
7. [완료] ETF 파일럿에서 검증한 연도·태그 필터를 Programming·Semiconductor·Reports·Economics·Computer Science로 확장했습니다. Market Brief의 특수 필터는 20번 계획에서 다룹니다.
