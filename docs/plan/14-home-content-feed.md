...
- 홈은 프로필, Finance·Semiconductor·Computing 허브 카드, 다짐 목록으로 구성됩니다.
- 상위 허브로는 이동할 수 있지만 개별 글로 바로 들어가는 영역은 없습니다.
- 공개 글 판정은 `getPublishedPosts()`가 단일 진입점이며 production에서 draft를 제외합니다.

## 목표

495개 이상의 글이 홈에서 실제 콘텐츠로 연결되도록 하되, 홈의 개인 소개 성격은 유지합니다.
...
1. 홈에서 사용할 정렬 규칙과 중복 제거 규칙을 순수 함수로 정의합니다.
2. `getPublishedPosts()`로만 데이터를 가져와 draft 노출을 막습니다.
3. 기존 카드가 홈 밀도에 적합한지 확인하고 필요하면 소형 카드를 분리합니다.
4. 각 섹션에 전체 보기 링크와 명확한 제목을 제공합니다.
5. 이미지가 있는 카드에는 lazy loading과 크기 속성을 유지합니다.
