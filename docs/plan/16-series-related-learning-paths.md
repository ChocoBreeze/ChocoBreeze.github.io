...
`prerequisiteSlugs`는 학습 경로 UI를 시작할 때 별도로 확정합니다.
...
7. [보류] 선수 글 데이터가 축적된 뒤 학습 경로 UI를 별도 단계로 추가합니다.
...
- Git Commands는 `series` 메타데이터를 사용하는 파일럿으로 전환했고, Git Internals·RAG는 기존 자동 수집 허브를 유지합니다.
