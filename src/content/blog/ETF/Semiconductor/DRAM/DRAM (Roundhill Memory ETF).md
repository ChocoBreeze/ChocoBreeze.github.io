...
기존 반도체 ETF가 NVIDIA, Broadcom, TSMC, ASML처럼 반도체 산업 전체에 넓게 투자한다면, DRAM은 훨씬 좁게 **메모리 사이클과 AI 메모리 수요**에 베팅하는 상품에 가깝습니다.
...
- **HBM**: AI GPU와 AI 가속기에 붙는 고대역폭 메모리
- **DRAM**: CPU, GPU, 서버, PC, 모바일에 쓰이는 휘발성 메모리
- **NAND / SSD**: 데이터 저장 장치와 스토리지 인프라
- **HDD / 스토리지**: 대용량 데이터 저장 수요와 연결

AI 시대에는 계산 칩만 중요한 것이 아닙니다. GPU가 아무리 빨라도 데이터를 충분히 빠르게 공급하지 못하면 병목이 생깁니다. 그래서 HBM과 고성능 메모리는 AI 서버 성능을 좌우하는 핵심 부품으로 평가됩니다.

DRAM은 이 지점에 집중합니다. 즉, 이 ETF는 "AI 반도체 전체"보다는 **AI 메모리 병목**에 더 가까운 투자 상품입니다.

## 3. 포트폴리오 구조

![DRAM ETF 상위 보유 종목](images/image.png)
...
![DRAM ETF 국가별 비중](images/image-1.png)
...
![DRAM ETF 자금 유입 추이](images/image-2.png)
...
![DRAM ETF 가격 흐름](images/image-3.png)
...
즉, DRAM ETF는 "반도체 장기 성장"보다 더 좁은 **메모리 업황 사이클**에 민감합니다.
...
DRAM의 핵심 차별점은 NVIDIA 중심이 아니라는 점입니다. AI 반도체 테마를 사고 싶지만 GPU 설계사보다 **HBM과 메모리 공급망**에 집중하고 싶을 때 고려할 수 있는 상품입니다.
...
DRAM은 AI 시대의 핵심 병목 중 하나인 **메모리**에 집중하는 ETF입니다. HBM, DRAM, NAND, 스토리지 기업에 직접적으로 노출되기 때문에 AI 인프라 투자가 메모리 기업 실적으로 이어진다고 보는 투자자에게는 매우 선명한 상품입니다.
...
요약하면, DRAM은 **"AI 반도체를 사고 싶다"보다 더 구체적으로 "AI 메모리 병목에 투자하고 싶다"는 관점의 ETF**입니다.
