...
COPX(Global X Copper Miners ETF)는 2010년 4월 19일부터 운영 중인 **글로벌 구리 광산 회사 전문 ETF 중 가장 오래되고 가장 큰 규모**다. 현재 순자산 \$4.56-5.81B, 보수료 0.65%, 41-47개 종목 보유로 **구리 수요 메가트렌드에 대한 검증된 진입로**를 제공한다.

COPX는 **"구리 광산 세계의 엘더 스테이츠맨이자 가장 검증된 선택"** 이다:

**검증된 성과**:

- 1년 수익: **66.76%** (SPY 18.48% 대비 +48.3% 우월)
- 10년 연간: **18.96%** (SPY 10.7% 대비 +8.3% 우월) - 검증됨
- 16년 역사: 2008 금융위기 생존, 2011-2015 구리 약세 견딤, COVID 극복
- 누적 수익: **103-135%** (since 2010)
...
**현 시점 평가**: COPX는 **"구리에 베팅하지만 극도의 위험을 피하고 싶은 투자자의 최우선 선택"** 이다. 신설 ETF(COPP, COPJ)보다 느리지만 안정성은 압도적이다.
...
| 항목 | 내용 |
| :-- | :-- |
| **공식명칭** | Global X Copper Miners ETF |
| **운용사** | Global X (Mirae Asset Global) |
| **티커** | COPX |
| **상장일** | 2010년 4월 19일 (16년 검증) |
| **순자산(AUM)** | 약 456-581억 달러 (극도로 거대) |
| **보수율** | 0.65% (COPP와 동일, COPJ보다 낮음) |
| **기초지수** | Solactive Global Copper Miners Total Return Index |
| **분배 주기** | 반연간 (매년 2회) |
| **보유 종목 수** | 41-47개 |
| **펀드 구조** | 지수 추종 (반연간 재조정) |
...
COPX는 **매우 체계적인 Solactive 지수 방식**을 추구한다:
...
- **대형주** (~70%): Lundin, First Quantum, Antofagasta, Glencore
- **중형주** (~20%): 다양한 개발/생산사
- **소형주** (~10%): 일부 소형 생산사
...
COPX의 가장 주목할 특징은 **4.75% 최대 가중치 제약**이다:


| 특성 | COPX | COPP | COPJ |
| :-- | :-- | :-- | :-- |
| **최대 가중치** | 4.75% | 무제한 (26%) | 무제한 (6.59%) |
| **최대 집중사** | Lundin 5.46% | FCX 26% | Taseko 6.59% |
| **상위 10개 비중** | 47.83% | 50.44% | ~50% |
| **위험도** | 낮음 | 높음 | 중간 |
...
COPX의 성과는 **장기 검증과 단기 탁월성**을 보여준다:


| 기간 | COPX | SPY | 격차 | 평가 |
| :-- | :-- | :-- | :-- | :-- |
| **1년** | 66.76% | 18.48% | +48.3% | 우월 |
| **10년** | 18.96% | 10.7% | +8.3% | 검증됨 |
| **16년 (since 2010)** | 103-135% | ~250%+ | 뒤짐 | 자산 별도 움직임 |
...
COPX는 **극도로 변동성 높은 포트폴리오**다:


| 해 | COPX 수익 | 특징 |
| :-- | :-- | :-- |
| **2010** | +15% | 정상 |
| **2011-2015** | -20% to -25% | 구리 약세 극심 |
| **2016** | +20% | 회복 시작 |
| **2017** | +40% | 강력한 해 |
| **2018** | -10% | 약간 약세 |
| **2019** | +25% | 강세 |
| **2020** | +45% | COVID 충격 후 회복 |
| **2021** | +20% | 안정적 |
| **2022** | -0.38% | 약간 약세 |
| **2023** | +7.38% | 약세 |
| **2024** | +4.37% | 매우 약세 |
...
| 항목 | COPX | COPP | COPJ |
| :-- | :-- | :-- | :-- |
| **출시 연도** | 2010 (16년) | 2024 (1년) | 2023 (2년) |
| **역사 증명** | ⭐⭐⭐⭐⭐ | ⭐⭐ | ⭐⭐ |
| **AUM** | \$4.56B (최대) | \$165M | \$83M |
| **가중치 제약** | 4.75% (최안전) | 무제한 (위험) | 무제한 (위험) |
| **1년 수익** | 66.76% | 72.19% | 137.4% |
| **10년 검증** | +18.96% | N/A | N/A |
| **변동성** | 20% | 23% | 35% |
| **배당 수익** | 1.23% (실제) | 2.13% (실제) | 10.13% (허위) |
| **2008 위기 생존** | ✓ 증명 | ? 미지수 | ? 미지수 |
| **추천 연령** | 45-70 | 40-55 | 30-40 |
| **포트폴리오 비중** | 7-15% | 5-10% | 2-5% |
| **위험 허용도** | 중간 | 중간-높음 | 높음 |
...
- **2008**: 구리 -70%, COPX 약 -50-60%
- **2011-2015**: 4년 연속 약세
- **2020 COVID**: 초기 충격 후 회복

**2026 위험**:

- **경기 침체**: 25% 확률
- **구리 -30-40%**: COPX -40-50% 가능
...
- **Glencore**: 석탄, 아연, 니켈 (석탄 = ESG 리스크)
- **BHP**: 철광석, 석탄 (구리 %만 12%)
- **순수성 문제**: 다금속이 구리 순수 베팅 희석
...
COPX는 **"구리 수요 슈퍼사이클의 가장 검증되고 신뢰할 수 있는 선택"** 이다.
...
**COPX는 "구리에 베팅하되 극도의 위험을 피하고 싶은 투자자의 최우선"이다.**

2025년 66% 수익은 **지속 가능할 가능성 높다** (구리 부족은 구조적):
...
**"COPX는 느리지만 확실하다. 신설 ETF(COPP, COPJ)의 극도의 수익률을 기대하지 말고, 대신 검증된 16년 역사와 \$4.56B 규모로부터 안심을 얻어라."**
...
10. **VEU** - 국제 주식 종합
11. **VEA** - 선진국 시장 전문
12. **COPJ** - 주니어 구리 광산 (극고위험)
13. **COPP** - 대형 구리 광산 (중위험)
14. **COPX** - 글로벌 구리 광산 (검증된 안정성)
