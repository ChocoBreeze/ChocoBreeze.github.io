...
## 개요
PNQI는 나스닥에 상장된 인터넷 관련 기업에 집중 투자하는 ETF로, Nasdaq CTA Internet Index를 추종한다. 검색 엔진, 인터넷 소매상거래, 소셜 미디어, 클라우드, 스트리밍 등 인터넷 비즈니스에 관여하는 미국 상장 기업들로 구성되며, Meta, Alphabet, Apple, Amazon, Microsoft 등 메가캡 인터넷 기업이 포트폴리오의 핵심을 이룬다. Morningstar는 이 펀드를 **Large Growth** 카테고리로 분류하며, 비용비율 0.60%에 AUM 약 $562M~$802M 수준의 중형 규모 ETF이다.[1][2][3][4][5]
...
| 항목 | 내용 |
|------|------|
| **최종 폴더** | `ETF/Internet/PNQI` |
| **대분류** | 테마 |
| **하위 분류** | 인터넷 |
| **핵심 전략** | Nasdaq CTA Internet Index를 추종하며 인터넷 비즈니스 관련 기업에 투자 |
| **레버리지·인버스** | 아니오 |
| **옵션 인컴 전략** | 아니오 |
| **분류 판단** | 이름에 Nasdaq이 포함되어 있지만 Nasdaq-100 대표지수 ETF가 아니라 인터넷 관련 기업에 투자하는 테마형 ETF로 분류하는 것이 적절하다. |

***
## 1. 기본 정보
| 항목 | 내용 |
|------|------|
| **정식 명칭** | Invesco NASDAQ Internet ETF |
| **티커** | PNQI (NASDAQ 상장) |
| **설정일** | 2008년 6월 12일[1][2] |
| **운용 기간** | 약 17년 10개월 |
| **운용사** | Invesco Ltd / Invesco Capital Management LLC[2] |
| **상장 거래소** | NASDAQ[6] |
| **추종 지수** | Nasdaq CTA Internet Index[2][7] |
| **지수 가중 방식** | Modified Market-Cap Weighted (수정 시가총액 가중)[8] |
| **순자산 규모(AUM)** | 약 $561.75M (2026.03 기준)[1] |
| **시가총액** | $674M |
| **현재가** | $47.26 (2026.03.11 기준) |
| **52주 범위** | $37.75 ~ $57.22 |
| **총 보유 종목 수** | 77개 |
| **P/E 비율** | 28.99 |
| **EPS** | $1.63 |
...
| **운용 방식** | 패시브 (인덱스 추종)[2] |
| **복제 방식** | 실물 복제 (Physical Replication)[2] |
...
- 상위 5개 종목의 최대 비중: 각 **8%** 캡[8]
- 나머지 종목의 최대 비중: 각 **4%** 캡[8]
- 캡 초과분은 나머지 종목에 비례 배분[8]
- 분기별 리밸런싱: 3월, 6월, 9월, 12월 셋째 금요일 이후 적용[8]

이 가중치 제한 메커니즘은 메가캡 기업의 과도한 집중을 완화하되, 시가총액 기반의 자연스러운 비중 배분을 유지한다.
### NAV 대비 괴리율
PNQI의 NAV 대비 프리미엄/할인율은 현재 약 **-0.01%~0.01%** 수준으로 극히 낮다. 2024년 전체 252 거래일 중 NAV 대비 프리미엄이 발생한 날은 121일, 할인이 발생한 날은 131일로, 대체로 NAV 근방에서 안정적으로 거래되었다. 괴리율이 0.25% 이상 벌어진 경우는 거의 없어 차익거래 메커니즘이 효과적으로 작동하고 있다.[2][10]
### 추적 성과
...
| 항목 | 내용 |
|------|------|
| **총 보수율 (Total Expense Ratio)** | 0.60%[1][2][3] |
| **순 보수율 (Net Expense Ratio)** | 0.60%[1] |
| **포트폴리오 회전율 (Turnover)** | 29%[13][9] |
| **수정 시가총액 가중 + 분기 리밸런싱** | 거래 비용 발생 요인[8] |
### 경쟁 ETF 대비 비용 비교
인터넷/테크 테마 ETF 중 PNQI의 0.60% 비용비율은 상대적으로 높은 수준이다. 주요 경쟁 ETF와의 비교는 다음과 같다:[14][15]

| ETF | 추종 지수 | 비용비율 | AUM |
|-----|-----------|----------|-----|
| **PNQI** | Nasdaq CTA Internet Index | 0.60%[1] | ~$562M[1] |
| **FDN** | Dow Jones Internet Composite Index | 0.52%[14] | ~$7.4B[16] |
| **ARKW** | ARK Next Gen Internet (Active) | 0.88%[16] | — |
| **QQQ** | Nasdaq-100 Index | 0.20% | ~$391B |
...
| 항목 | 수치 |
|------|------|
| **일평균 거래량 (최근 3개월)** | 55,921주 |
| **평균 거래량 (장기)** | 103,377주 |
| **일평균 거래대금 (최근 3개월)** | $2,520,146 |
| **30일 평균 거래량 (YCharts)** | 30,295주[13] |
| **NAV 대비 할인/프리미엄** | -0.01% ~ -0.04%[2][13] |
...
상위 10종목 합산 비중은 **62.45%**로, 메가캡 인터넷 기업에 대한 집중도가 높다. 특히 상위 5개 종목(META, GOOG, AAPL, AMZN, MSFT)만으로 약 42.3%를 차지하며, 지수의 8% 캡 규칙이 적용된 결과이다.[8]
### 섹터별 배분 현황
...
### 리밸런싱 주기
펀드와 지수 모두 **분기별(Quarterly)** 리밸런싱 및 재구성을 실시한다. 리밸런싱은 매년 2월, 5월, 8월, 11월 말일의 종가와 총 발행주식수를 기준으로 계산하며, 3월, 6월, 9월, 12월 셋째 금요일 이후 장 개시 시점에 적용된다.[7][8]
...
| **표준편차 (1년 연환산)** | 24.49% |
| **표준편차 (3년 연환산)** | 25.49% |
| **표준편차 (FT.com, 1년)** | 18.59%[11] |
| **샤프 비율 (1년)** | 0.84 |
| **샤프 비율 (3년)** | 0.86 |
| **샤프 비율 (FT.com, 1년)** | 1.15[11] |
| **최대 낙폭 (2020~)** | -59.69% |
| **최대 낙폭 (설정 이래)** | -59.70%[22][14] |
| **베타 (5년)** | 1.15[23] |
| **베타 (1년, FT.com)** | 1.59[11] |
| **베타 (StockAnalysis)** | 1.21[6] |
| **알파 (1년, FT.com)** | +3.94%[11] |
| **SPY 대비 상관계수** | 0.86[24] |
최대 낙폭 **-59.69%**는 2022년 11월(금리 인상 + 인터넷 성장주 매도) 시점에 기록되었으며, 이는 FDN의 -61.55%와 유사한 수준이다. 베타가 1.15~1.59 범위로, S&P 500 대비 1.15~1.59배 높은 시장 민감도를 보인다.[6][11][14][23]

***
## 7. 배당 정보
| 항목 | 내용 |
|------|------|
| **배당 수익률 (TTM)** | 0.02%[6][25] |
| **연간 배당금** | $0.009 ~ $0.01[6][25] |
| **배당 지급 주기** | 비정기적 (거의 없음)[25] |
| **배당성향 (Payout Ratio)** | 0.49%[6] |
### 배당 이력
PNQI는 사실상 **무배당** ETF에 가깝다. 최근 배당 기록은 다음과 같다:[4][25]
...
17년 운용 기간 중 배당이 단 4회에 불과하며, 금액도 극히 소규모이다. 이는 포트폴리오를 구성하는 인터넷 성장주들이 배당보다 재투자를 선호하는 특성에 기인한다. PNQI는 **자본 이득(Capital Gain) 중심 ETF**로, 배당 소득을 기대하는 투자자에게는 적합하지 않다.[26][25]

***
## 8. 리스크 요소
### 베타 계수 및 시장 민감도
PNQI의 5년 베타는 **1.15**, 1년 베타는 **1.59**, StockAnalysis 기준 베타는 **1.21**로, 시장 대비 높은 변동성을 보인다. 특히 최근 1년 베타(1.59)가 장기 베타(1.15)보다 높아, 최근 시장 변동에 더욱 민감하게 반응하고 있다. SPY 대비 상관계수는 0.86으로 상당히 높은 수준이다.[6][11][24][23]
### 섹터 집중도 리스크
기술(37.3%), 커뮤니케이션(33.8%), 경기소비재(25.7%) 3개 섹터가 전체의 **96.7%**를 차지하는 극단적 집중 구조이다. 이는 다음과 같은 리스크를 초래한다:[17]

- **규제 리스크**: 빅테크 반독점 규제, 데이터 프라이버시 법률 강화가 포트폴리오 전반에 영향
- **금리 민감도**: 성장주 중심이므로 금리 상승 시 밸류에이션 압축이 심각 (2022년 -47.92% 사례)
- **경기 사이클**: 광고, 전자상거래, 구독 서비스 등이 소비 둔화에 민감
### 집중도 및 대형주 편중 리스크
...
| 항목 | PNQI | FDN | ARKW |
|------|------|-----|------|
| **추종 전략** | Nasdaq Internet Index (패시브) | Dow Jones Internet Composite (패시브) | 차세대 인터넷 (액티브)[16] |
| **비용비율** | 0.60%[1] | 0.52%[14] | 0.88%[16] |
| **AUM** | ~$562M[1] | ~$7.4B[16] | — |
| **보유 종목 수** | 77개 | ~40개[15] | — |
| **1년 수익률** | +33.83%[14] | +39.08%[14] | — |
| **5년 연환산** | +7.50%[14] | +9.11%[14] | — |
| **10년 연환산** | +13.27%[14] | +14.22%[14] | — |
| **샤프 비율** | 1.47[14] | 1.62[14] | — |
| **최대 낙폭** | -59.70%[14] | -61.55%[14] | — |
| **비미국 종목** | 포함 (BABA, MELI 등)[15] | 미포함[15] | — |
