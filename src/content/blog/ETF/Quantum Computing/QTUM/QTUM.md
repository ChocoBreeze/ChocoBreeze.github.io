...
**QTUM (Defiance Quantum ETF)**은 양자 컴퓨팅 및 머신러닝 분야의 글로벌 혁신 기업에 투자하는 업계 최초의 테마형 상장지수펀드입니다. Defiance ETFs에서 운용하며 BlueStar® Quantum Computing and Machine Learning Index (BQTUM)를 추종합니다.[^1]


| 항목 | 내용 |
| :-- | :-- |
| **티커** | QTUM (NASDAQ)[^2] |
| **설정일** | 2018년 9월 4-5일[^3][^4] |
| **운용사** | Defiance ETFs |
| **상장거래소** | NASDAQ |
| **추종 지수** | BlueStar Quantum Computing and Machine Learning Index (BQTUM)[^5] |
| **현재 AUM** | \$3.2B (2026년 1월 기준)[^6] |
| **보유 종목 수** | 86개[^7] |
...
**2026년 1월 5일 기준 QTUM의 성과는 탁월한 성장세를 보여줍니다:**

![QTUM 기간별 수익률](images/image.png)
...
다만 **추적 오차**가 상당한 수준입니다. 1년 기준 17.12-17.32%의 추적 오차를 보이고 있으며, 이는 펀드의 실제 성과가 지수를 추종하는 과정에서 상당한 편차를 갖고 있음을 의미합니다. 추적 차이(Tracking Difference)의 중앙값이 -0.73%인 점을 고려하면, 운영 비용, 배당 처리, 리밸런싱 등에서 연 약 0.7% 정도의 손실이 발생하고 있습니다.[^10][^11]

**NAV 대비 시장가격 괴리율**은 매우 양호합니다. 평균 프리미엄/디스카운트가 0.10% 수준으로, ETF의 공정가가 시장 가격과 거의 일치하고 있음을 보여줍니다.[^11]

### 3. 비용 구조

| 비용 항목 | QTUM | 업계 평균 |
| :-- | :-- | :-- |
| **총 관리비(Expense Ratio)** | 0.40%[^6][^12] | 0.50-0.80% |
| **추적 차이(Tracking Difference)** | -0.73% (중앙값)[^11] | -0.30~-1.00% |
| **포트폴리오 회전율** | 1.4%[^13] | 2-5% |
| **평균 호가 스프레드** | 0.07%[^13] | 0.10-0.20% |
...
| 유동성 지표 | 수치 | 평가 |
| :-- | :-- | :-- |
| **평균 일 거래량** | 374,000-407,000주[^6][^13] | 우수 |
| **평균 일 거래대금** | \$44-46M[^13] | 우수 |
| **호가 스프레드** | 0.07%[^13] | 매우 우수 |
| **AUM 추이** | 상승세[^15][^9] | 강화 중 |
...
![QTUM 섹터별 배분 현황](images/image-1.png)
...
![QTUM 상위 10대 보유 종목](images/image-2.png)
...
- **단기(1-6개월)**: 높은 변동성 속에서도 상승세 유지. 1개월 2.43%, 3개월 20.36%, 6개월 68.81%
- **중기(1-3년)**: 연평균 38.18% (3년)로 매우 우수한 수익률
- **장기(5년)**: 연평균 23.70%, 상장 이후 누적 370.42%[^10]
...
- **샤프 지수**: 0.93 (양호한 수준)[^19]
- **칼마 비율**: 0.62 (수익 vs 최대 낙폭 비율)[^19]
- **소르티노 지수**: 1년 6.79 (하락 위험 조정 수익률)[^10]
...
| 배당 지표 | 수치 |
| :-- | :-- |
| **배당 수익률** | 0.65-1.13%[^20][^21] |
| **연간 배당액** | \$1.06-1.11[^12][^20] |
| **배당 주기** | 분기별[^21] |
| **최근 배당액** | \$0.425 (2025년 12월)[^22] |
| **1년 배당 증가율** | 33-45%[^20][^23] |
...
![QTUM 주요 리스크 지표](images/image-3.png)

QTUM ETF 주요 리스크 지표

**시스템적 위험:**

- **베타**: 0.94-1.30 - 시장과 유사하거나 약간 높은 민감도를 보입니다. 1.30에 가까운 경우 시장이 1% 하락할 때 QTUM은 1.3% 하락할 수 있음을 의미합니다.[^6][^24]
- **S\&P 500 상관계수**: 0.67-0.80 - 광범위한 시장과의 상관관계가 적당한 수준으로, 포트폴리오 분산화 효과를 제공합니다.[^25][^24]

**변동성 및 낙폭:**

- **표준편차**: 약 27% - 기술주 ETF(예: QQQ)의 20-25% 대비 높은 수준입니다.[^19]
- **최대 낙폭**: -38.44% - 2020년 코로나 팬데믹 당시의 시장 하락장에서 나타났습니다. 광범위한 기술주 지수 대비 더 큰 낙폭을 보일 수 있음을 의미합니다.[^19]
...
1. **강한 성장 모멘텀**: 2024년 이후 지속적인 자금 유입과 55% 이상의 연 수익률
2. **경쟁력 있는 비용 구조**: 0.40%의 낮은 관리비와 0.07%의 호가 스프레드
3. **우수한 유동성**: 일 거래대금 \$44-46M으로 대규모 자금도 거래 가능
4. **분산된 포트폴이오**: 86개 종목으로 개별 종목 리스크 분산
5. **글로벌 노출**: 미국, 캐나다, 유럽, 아시아의 양자 기업 포괄

**QTUM의 위험:**

1. **높은 변동성**: 표준편차 27%, 최대 낙폭 -38.44%
2. **테마 집중도 리스크**: 양자 컴퓨팅 상용화 시점 불확실
3. **높은 추적 오차**: 1년 기준 17%+ 로 지수 추종도가 낮음
4. **소형주 노출**: 양자 컴퓨팅 신생 기업의 높은 비중
5. **기술 섹터 편중**: 67% 이상이 정보기술 섹터
