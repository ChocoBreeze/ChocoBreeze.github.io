...
> **작성 기준일:** 2026년 5월 3일 | **데이터 출처:** ProShares 공식 사이트, StockAnalysis, ETFdb, PortfoliosLab, TrendSpider 등

## ETF 분류

| 항목 | 내용 |
|------|------|
| **최종 폴더** | `ETF/Leveraged Inverse/Nasdaq-100/QID` |
| **대분류** | 레버리지·인버스 |
| **하위 분류** | Nasdaq-100 레버리지 인버스 |
| **핵심 전략** | Nasdaq-100 지수의 일일 수익률을 -2배로 추종 |
| **레버리지·인버스** | 예, 일일 -2x 인버스 |
| **옵션 인컴 전략** | 아니오 |
| **분류 판단** | Nasdaq-100 노출이 있지만 일반 대표지수 ETF가 아니라 일일 -2배 인버스 구조가 핵심이므로 레버리지·인버스 폴더에 우선 분류한다. |
...
| 순 보수(Net Expense Ratio) | **0.95%**[1][2] |
| 추종 목표 | 나스닥-100 지수 일간 수익률의 **-2배 (-2x)** |
| 추종 지수 | Nasdaq-100 Index® |
| 분배 주기 | 분기배당 |
| 레버리지 유형 | 일일 재설정 역방향(-2x) 레버리지 ETF |

QID(ProShares UltraShort QQQ)는 2006년 7월 11일 설정된 미국의 레버리지 인버스 ETF로, 나스닥-100 지수의 **일간 수익률** 대비 -2배 수익을 목표로 설계된 파생상품 기반 펀드입니다. 나스닥 100이 하루에 1% 하락하면 QID는 이론적으로 2% 상승하고, 나스닥 100이 1% 상승하면 QID는 약 2% 하락합니다.[3][4][5]
...
### 일일 레버리지 리셋 메커니즘
레버리지 배율은 **매일 영업 종료 시 재설정**됩니다. 이는 특정 기간의 누적 수익률이 해당 기간 나스닥-100 수익률의 정확히 -2배가 되지 않는다는 것을 의미합니다. 시장이 횡보하거나 변동성이 높을 때 '변동성 감쇠(Volatility Decay)' 또는 '베타 슬리피지(Beta Slippage)'가 발생하여 장기 보유 시 큰 손실을 야기합니다.[8][9][10][11]
...
QID는 주가 희석으로 인해 **2024년 4월 10일 1:5 역분할**을 시행했습니다. 역분할 이후 주가가 5배로 상승하고 주식 수는 1/5로 감소했으며, 이는 주가 하락에 따른 관리 목적의 조치였습니다. CUSIP 번호도 새로 부여되었습니다(74349Y829).[16][17]
...
- 섹터별 배분은 나스닥-100과 동일하지만, **역방향(-2x) 노출** 적용
...
| **2022** | **+66.30%** | **+36.40%** | **나스닥 -32% 급락** |
| 2023 | -57.19% | -32.01% | 나스닥 강세 반등 |
| 2024 | -34.06% | -15.68% | 나스닥 강세 |
| 2025 | -34.97% | -15.51% | 나스닥 강세 |
| 2026 YTD | +12.92% | +7.10% | 나스닥 약세[18] |

2022년을 제외하면 나스닥 강세장 지속으로 QID는 매년 대규모 손실을 기록했습니다. 2017년 시작 기준 $1,000 투자 시 2025년 말 약 **$13 수준**으로 하락한다는 것을 누적 복리 계산으로 확인할 수 있습니다.[18]
...
- 나스닥 100 누적 변화: 1.10 × 0.90 = 0.99 → **-1% 손실**
- QID(-2x) 이론값: (1 - 0.20) × (1 + 0.20) = 0.96 → **-4% 손실** (단순 -2x가 아닌 +2% 이상 과대 손실)
...
일간 단위에서 QID는 목표인 -2x를 매우 정확하게 추종합니다. 그러나 다수 일 보유 시 일일 복리 효과로 인해 누적 수익률은 나스닥-100의 -2배와 크게 달라집니다. 이것은 설계상의 결함이 아니라 **일일 레버리지 리셋이라는 구조적 특성**입니다.[8][22]
...
**투자자 경고:** ProShares를 포함한 레버리지/인버스 ETF 발행사들은 공식 자료에서 이 상품들이 **단기 트레이딩 목적**으로만 적합하며 장기 보유에는 부적절하다고 명시하고 있습니다.[8][14]

***
## 10. 활용 전략 및 경쟁 ETF 비교
### QID의 적절한 활용 사례
1. **단기 헤지(Short-Term Hedge):** 기술주 중심 포트폴리오의 하락 리스크를 단기적으로 헤지할 때 사용. 보유 포트폴리오의 10~20%를 QID로 헤지하는 전략이 일반적입니다.[25]
2. **단기 방향성 거래:** 나스닥 100 급락이 예상될 때 1~5일 내외의 단기 포지션 진입.[4]
3. **변동성 이벤트 대비:** 주요 경제 지표 발표, FOMC 회의, 실적 시즌 등 단기 불확실성 구간에서 활용.
### 경쟁 인버스 ETF 비교
| 항목 | PSQ | **QID** | SQQQ |
|------|-----|---------|------|
| 레버리지 | -1x | **-2x** | -3x |
...
| AUM | ~$2억 | **~$2.8억** | ~$25억 |
| 일 거래량 | 낮음 | **중간** | 매우 높음 |
| 2022 수익률 | +36.40% | **+66.30%** | +49.91%[20] |
| 2025 수익률 | -15.51% | **-34.97%** | -51%+ |
| 2026 YTD | +7.10% | **+12.92%** | ~+18%[14] |
| 10년 연환산 | -17.21% | **-35.76%** | ~-52.7%[14] |
| 샤프 (5Y) | -0.78 | **-0.84** | 더 낮음[18] |
| 변동성 감쇠 강도 | 낮음 | **중간** | 높음 |
| 적합 투자 기간 | 1~수일 | **1~수일** | 1~수일 |
...
**장기 투자로는 절대 적합하지 않습니다.** 2017년부터 9년 보유 시 $1,000 투자금이 약 $13으로 소멸될 만큼, 레버리지 감쇠와 방향성 리스크가 복합적으로 작용하여 자산이 사실상 전소됩니다. ProShares 역시 공식 문서에서 이 상품이 **단기 거래 및 헤지 목적** 외에는 사용하지 말 것을 명시하고 있습니다.[8]

**적합한 투자자:** 나스닥 100의 단기 하락에 강한 확신이 있는 전문 트레이더, 기술주 포트폴리오 단기 헤지 목적의 기관 투자자, 시장 급락 이벤트를 활용하는 단기 매매자.

**부적합한 투자자:** 일반 장기 투자자, 변동성 감쇠를 이해하지 못한 투자자, 투자 원금 보전이 필요한 투자자.
