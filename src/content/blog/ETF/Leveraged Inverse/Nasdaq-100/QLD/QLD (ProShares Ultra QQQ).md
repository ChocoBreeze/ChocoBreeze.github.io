...
- **순자산(AUM)**: \$10.87B~\$11.16B (2026년 1월 기준)[^3][^4][^11]
- **총 보수(TER)**: 0.95% (Net), 0.98% (Gross)[^12][^13][^1][^3]
- **보수 면제**: 2026년 9월 30일까지 계약적 면제 적용[^13][^1]
- **보유 종목 수**: 119개 (주식 + 파생상품)[^8]
- **레버리지 배수**: 2x (일일 수익률 기준)[^2][^1][^3]
- **상장거래소**: NYSE Arca (NYSEARCA)
...
**운용 기간**: 2006년 6월 19일 설정 이후 현재까지 약 20년 운용[^3][^11][^1]

### 1.3 추종 지수명

QLD는 **Nasdaq-100 Index**의 **일일 수익률 2배**를 추종합니다. 중요한 점은 "일일(daily)" 수익률 기준이라는 것으로, 누적 수익률은 복리 효과 및 변동성 감쇠로 인해 2배와 다를 수 있습니다. 예를 들어 Nasdaq-100이 하루 +10% 상승하면 QLD는 약 +20% 상승을 목표로 하지만, 여러 날에 걸친 누적 수익률은 단순히 2배가 아닙니다.[^1][^2][^3][^5][^14][^6]

**일일 레버리지 리셋(Daily Reset)**
...
QLD는 **NYSE Arca** 거래소에 상장되어 있으며, 티커 심볼은 "QLD"입니다. 레버리지 ETF 중에서는 높은 거래량을 보이며, 옵션 거래도 활발하게 이루어집니다.[^1][^3][^11]
...
QLD의 추적오차는 **일일 수익률 기준**으로는 낮은 수준이나, **누적 수익률 기준**으로는 상당한 차이가 발생합니다. ProShares 공식 데이터에 따르면, QLD는 일일 목표(Nasdaq-100의 2배)를 충실히 달성하지만, 변동성 감쇠(volatility decay) 효과로 장기 누적 수익률은 2배 미만입니다.[^14][^15]

**일일 vs 누적 추적 차이**

- **일일**: Nasdaq-100 +1% → QLD 약 +2% (목표 달성)
- **누적 (1년)**: Nasdaq-100 +16.02% → QLD +18.10% (1.1배, 2배 미만)[^15]
- **누적 (10년)**: Nasdaq-100 +17.94% 연평균 → QLD +27.14% 연평균 (약 1.5배)[^15]


### 2.2 추적 차이(Tracking Difference)

QLD의 장기 추적 차이는 주로 **변동성 감쇠**와 **복리 효과**에서 발생합니다. 학술 연구에 따르면, QLD는 설계상 Nasdaq-100의 일일 수익률 2배를 추구하지만, 누적 장기 성과는 2배 벤치마크를 체계적으로 하회합니다(systematically lags).[^5][^14][^6]
...
QLD의 시장가격은 순자산가치(NAV)와 밀접하게 연동됩니다. ProShares 공식 데이터에 따르면 30일 중간 호가 스프레드가 **0.01%** 수준으로, 괴리율도 매우 낮습니다. 다만 레버리지 ETF 특성상 일반 ETF 대비 괴리율이 약간 높을 수 있으나, 여전히 합리적 범위 내입니다.[^1][^16][^17]
...
QLD의 총 보수는 **0.95% (Net), 0.98% (Gross)**입니다. ProShares는 총 연간 운용 비용이 평균 일일 순자산의 0.95%를 초과할 경우 그 초과분을 면제하거나 보전하기로 합의했으며, 이 조건은 **2026년 9월 30일까지** 유효합니다.[^1][^3][^12][^13]
...
QLD의 포트폴리오 회전율은 공개되지 않았으나, 레버리지 ETF 특성상 **높을 것**으로 예상됩니다. 매일 레버리지 비율을 재조정하고 파생상품 포지션을 롤링해야 하므로, 일반 패시브 ETF(QQQ 7.98%) 대비 훨씬 높은 회전율을 보일 것입니다.[^6]
...
QLD의 호가 스프레드는 **0.01% (1 bps)**로 매우 낮은 수준입니다. 이는 높은 거래량(일평균 5~6백만 주)과 활발한 마켓메이킹 덕분입니다. 개인 투자자도 기관투자자와 유사한 수준의 낮은 거래 비용으로 매매할 수 있습니다.[^1][^7]
...
2026년 1월 기준 QLD의 일평균 거래량은 **4.98~6.63백만 주** 수준입니다. 이는 레버리지 ETF 중에서는 높은 수준이며, 대형 기관투자자도 원활히 거래할 수 있는 유동성을 제공합니다.[^1][^7]
...
일평균 거래량 6백만 주에 주가 약 \$73을 곱하면, 일평균 거래대금은 약 **\$438백만** 수준으로 추정됩니다. 이는 레버리지 ETF로서는 충분한 유동성이며, 수백만~수천만 달러 규모의 포지션도 시장 충격 없이 거래 가능합니다.

### 4.3 호가 스프레드 평균

QLD의 평균 호가 스프레드는 **0.01% (1 bps)**입니다. \$73 주가 기준으로 약 \$0.007의 매수-매도 차이를 의미하며, 사실상 무시할 수 있는 수준입니다.[^1][^7]
...
QLD는 2배 레버리지를 구현하기 위해 **파생상품(스왑, 선물)**을 적극 활용합니다. 2024년 12월 기준 파생상품 비중은 약 **106.72%**로, 포트폴리오의 100%를 초과합니다(레버리지 효과).[^1][^8]

**주요 파생상품 포지션 (2024년 12월)**[^8][^1]
...
상위 10종목 비중은 약 **39.51%**이며, 파생상품 포지션을 합치면 100%를 초과합니다.[^8]
...
기술 관련 섹터(Technology + Communication Services)가 약 **70%**를 차지하며, 기술주 집중 리스크가 매우 높습니다. 레버리지 효과로 기술주 변동성이 약 2배 증폭됩니다.[^19]

### 5.4 국가별/지역별 분산 (해당 시)

QLD는 **미국 중심 ETF**로, 미국 주식이 86.05%, 비미국 주식이 3.28%를 차지합니다. 지역별 분산은 사실상 없으며, 미국 기술주에 집중된 포트폴리오입니다.[^11]

**자산 배분 (Morningstar)**[^11]
...
QLD는 **매일(daily)** 레버리지 비율을 2x로 재조정합니다. 이는 레버리지 ETF의 핵심 메커니즘으로, 다음과 같이 작동합니다:[^6]
...
- **5년 연평균**: +18.27~26.27%
- **10년 연평균**: +27.14~30.73%
- **설정 이후 연평균**: +24.39~24.62%

설정 이후 \$10,000 투자 시 2026년 기준 약 \$640,000로 성장하여, 연평균 +24.55%의 복리 수익률을 달성했습니다. 이는 QQQ의 연평균 +10.43% 대비 약 2.4배입니다.[^21]

### 6.2 벤치마크 대비 초과 수익률

QLD의 벤치마크(Nasdaq-100) 대비 성과는 **장기적으로 2배 미만**입니다. 변동성 감쇠 및 보수(0.95%) 효과로 누적 수익률이 제한됩니다.[^14][^15]

**벤치마크 대비 성과 (ProShares 연례 보고서)**[^15]
...
**일일 수익률**: 목표대로 약 2배
**누적 수익률**: 변동성 감쇠로 2배 미만 (장기적으로 1.3~1.5배)[^15]
...
**10년 Dollar Cost Averaging 비교 (\$120,000 총 투자)**[^24]
...
QLD의 샤프 지수는 **1.32**로, 리스크 조정 수익률이 양호한 수준입니다. 이는 QQQ(2.03)보다 낮지만, 레버리지 ETF로서는 합리적입니다. TQQQ(1.86)와 비교하면 QLD가 약간 낮습니다.[^10]
...
QLD의 연환산 변동성(표준편차)은 **36.1%**로, S\&P 500의 약 **3배**입니다. 이는 2배 레버리지보다 높은 수준으로, 복리 효과 및 일일 리밸런싱의 영향입니다.[^20]
...
QLD의 역사적 최대 낙폭은 **-72.88%**로, 2008년 금융위기 시기에 기록되었습니다. 최근 5년 기준 최대 낙폭은 **-63.68%**입니다.[^20][^9][^23]
...
QLD의 낙폭은 벤치마크의 약 **1.9배** 수준으로, 레버리지 효과를 충실히 반영합니다. 회복 기간은 약 10~24개월로, 장기간 손실 상태가 지속될 수 있습니다.[^9][^10]
...
QLD는 분기배당 ETF로, 2026년 1월 기준 **배당 수익률 0.16~0.24%**를 제공합니다. 이는 레버리지 비용(차입 이자) 및 파생상품 특성으로 인해 낮은 수준입니다.[^25][^26][^27]
...
QLD는 **분기배당**을 지급하며, 3월, 6월, 9월, 12월에 배당락일이 설정됩니다. 배당금은 분기별로 큰 변동성을 보이며, 이는 파생상품 손익 및 자본이득 실현 타이밍에 따라 달라집니다.[^25][^26]

**최근 6분기 배당 이력**[^26][^27][^25]
...
QLD의 배당 성장률은 **-14.06% (1년 기준)**로, 2025년 배당금이 전년 대비 감소했습니다. 레버리지 ETF는 배당보다 자본이득이 주요 수익 원천이므로, 배당 성장은 중요하지 않습니다.[^25][^26]
...
QLD의 베타는 **2.29~2.39** (S\&P 500 대비)로, 시장 평균의 약 **2.4배** 변동성을 보입니다. 이는 2배 레버리지를 충실히 반영하며, 시장이 10% 상승(하락) 시 QLD는 평균 23.9% 상승(하락)함을 의미합니다.[^3][^26]
...
QLD는 S\&P 500 및 미국 주식 시장과 **매우 높은 상관계수**를 보입니다. S\&P 500과의 상관계수는 **+0.94**로, 거의 완벽한 동조화를 보입니다.[^20]
...
QLD의 가장 큰 리스크는 **기술주 집중도**와 **레버리지 증폭 효과**입니다. Technology 섹터가 55% (일부 출처 63%)를 차지하며, Communication Services를 포함하면 약 **70%**가 기술 관련 섹터입니다. 레버리지 효과로 기술 섹터 침체 시 QLD는 극심한 타격을 받을 수 있습니다.[^11][^19]
...
QLD의 핵심 리스크는 **변동성 감쇠(volatility decay)**입니다. 일일 레버리지 리셋으로 인해 횡보장 및 변동성 높은 시장에서 장기 성과가 저하됩니다.[^5][^14][^6][^10]
...
- **TQQQ** (ProShares UltraPro QQQ): 3x 레버리지, 0.86~0.97% TER[^9]
- **QQQ** (Invesco QQQ Trust): 1x 패시브 추종, 0.18% TER
- **SSO** (ProShares Ultra S\&P 500): 2x S\&P 500 레버리지, 0.89% TER[^19]
...
- **QLD 선택 시**: 2x 레버리지로 충분, 리스크 관리 중시, TQQQ 대비 낮은 변동성
- **TQQQ 선택 시**: 최대 수익률 추구, 극단적 리스크 감내 가능, 단기 강세장 확신


### 9.3 QLD vs QQQ: 레버리지 vs 패시브

**10년 Dollar Cost Averaging 비교**[^24]
...
- **QLD 선택 시**: 단기~중기 강세장 전망, 높은 변동성 감내 가능, 적극적 모니터링 가능
- **QQQ 선택 시**: 장기 투자 (Buy \& Hold), 안정적 성과 선호, 변동성 회피


### 9.4 투자 전략 차이

- **QLD**: 일일 2x 레버리지, 파생상품 활용, 일일 리밸런싱, 단기~중기 투자[^1][^6][^9]
- **TQQQ**: 일일 3x 레버리지, 극단적 리스크-수익, 초단기 투자[^9][^28]
- **QQQ**: 패시브 추종, 낮은 보수, 장기 투자[^29]
...
QLD는 설계상 **단기~중기 전술적 투자**에 최적화되어 있습니다.[^4][^9][^10]

**투자 기간별 적합성**

- **1일~1주**: 매우 적합 (일일 2x 목표 달성)
- **1주~3개월**: 적합 (단기 추세 포착)
- **3개월~6개월**: 전술적 투자 가능[^4]
- **6개월~1년**: 주의 필요 (변동성 감쇠 시작)
- **1년 이상**: 부적합 (변동성 감쇠 심화)[^6][^10]
...
1. **단기 트레이더**: 일일~주간 거래
2. **전술적 투자자**: 3~6개월 시계열 투자[^4]
3. **높은 리스크 감내력**: MDD -60%+ 수용 가능
4. **적극적 모니터링 가능**: 일일 포트폴리오 체크
5. **Nasdaq-100 강세장 확신**: 기술주 상승 전망
6. **옵션 거래 경험자**: 파생상품 이해도 높음

**부적합한 투자자**

1. **장기 투자자 (Buy \& Hold)**: 변동성 감쇠로 성과 저하[^6][^10]
2. **보수적 투자자**: 높은 변동성 부담
3. **은퇴 계좌 (IRA, 401k)**: 장기 보유 목적 부적합
4. **변동성 회피 투자자**: 표준편차 36% 부담
5. **자산 배분 포트폴리오**: 레버리지로 분산 효과 상쇄
6. **금융 문맹(Financial Illiteracy)**: 레버리지 메커니즘 이해 부족

### 10.3 리스크 관리 전략

QLD 투자 시 **엄격한 리스크 관리**가 필수적입니다.[^9]
...
- **강세 시나리오 (확률 40%)**: Nasdaq-100 +20% → QLD 약 +36~40% (변동성 감쇠 감안)
- **중립 시나리오 (확률 40%)**: Nasdaq-100 0~5% → QLD 0~5% (변동성 감쇠로 언더퍼폼)
- **약세 시나리오 (확률 20%)**: Nasdaq-100 -15% → QLD 약 -28~30%

**투자 권고**: **HOLD (보유) - 단기 전술적 투자만 권장**

QLD는 단기~중기 강세장 전망 시 매력적이나, 장기 투자에는 부적합합니다. 2026년 변동성이 높을 것으로 예상되므로, 변동성 감쇠 리스크에 유의해야 합니다. 투자자는 다음 원칙을 준수해야 합니다:[^6][^9][^10]

1. **투자 기간 3~6개월 이내 제한**[^4]
2. **포트폴리오의 5~10% 이내 배분**[^9]
3. **-15~-20% 손절매 설정**[^9]
4. **강세장 확신 시에만 진입**
5. **출구 전략 사전 수립**
...
1. **일일 2배 레버리지**: 강세장에서 벤치마크의 약 2배 수익[^1][^9]
2. **우수한 유동성**: 일평균 5~6백만 주, 호가 스프레드 0.01%[^7][^1]
3. **TQQQ 대비 낮은 변동성**: 2x vs 3x, MDD -63% vs -81%[^9][^23]
4. **옵션 거래 가능**: 전략적 헤징 및 수익 증대 가능[^1]
5. **대형 AUM (\$11B+)**: 안정성 및 유동성 보장[^3][^4]
6. **20년 운용 실적**: 검증된 레버리지 메커니즘[^3][^1]

### 11.2 약점 (Weaknesses)

1. **높은 보수 (0.95%)**: QQQ(0.18%) 대비 약 5배[^3][^12]
2. **매우 높은 변동성**: 베타 2.39, 표준편차 36%[^20][^3]
3. **변동성 감쇠**: 장기 성과 2배 미만, 횡보장에서 손실 누적[^5][^14][^6]
4. **최대 낙폭 -60%+**: 극심한 손실 가능, 회복 기간 장기[^22][^20]
5. **일일 리셋**: 누적 수익률 불확실성[^6]
6. **파생상품 리스크**: 거래상대방 리스크 존재[^10]
7. **낮은 배당 수익률 (0.16%)**: 배당 투자자 부적합[^25]
8. **세금 비효율**: 단기 자본이득 과세 가능성

### 11.3 기회 (Opportunities)

1. **2026년 금리 인하**: 기술주 밸류에이션 지지
2. **AI 테마 지속**: 상위 보유 종목 (NVDA, MSFT, AAPL) 수혜
3. **단기 강세장 포착**: 전술적 투자 기회
4. **변동성 증가**: 단기 트레이더에게 기회

### 11.4 위협 (Threats)

1. **기술주 버블 붕괴**: 밸류에이션 조정 시 -60% 이상 급락 가능
2. **AI 투자 ROI 실망**: 실적 미달 시 상위 종목 급락
3. **규제 리스크**: 빅테크 반독점 규제 강화
4. **경기 침체**: 기술주 선행 하락, 레버리지로 손실 증폭
5. **횡보장 지속**: 변동성 감쇠로 장기 손실 누적[^5][^6]

### 11.5 최종 투자 등급

**투자 등급: HOLD (보유) - 단기 전술적 투자만 권장**
...
3. **손절매**: -15~-20% 설정[^9]
4. **진입 조건**: 강세장 확신 시에만
5. **모니터링**: 일일 포트폴리오 체크 필수

**대안 제안**

- **장기 투자자**: QQQ(1x) 권장
- **중간 레버리지**: QLD(2x) 유지
- **극단적 수익 추구**: TQQQ(3x) 고려 (초단기만)

QLD는 전문 트레이더 및 전술적 투자자에게는 유용한 도구이지만, 일반 투자자의 은퇴 계좌나 장기 포트폴리오에는 **절대 부적합**합니다. 투자 전 레버리지 메커니즘, 변동성 감쇠, 극단적 리스크를 충분히 이해하고, 엄격한 리스크 관리 하에 투자해야 합니다.[^10][^6][^9]
...
**작성 기준일**: 2026년 1월 31일
**데이터 출처**: ProShares, Morningstar, YCharts, Yahoo Finance, StockAnalysis, Barchart, Nasdaq, 학술 연구(SSRN), 기타 금융 데이터 제공업체

**중요 면책 조항**: 본 보고서는 정보 제공 목적으로 작성되었으며, 투자 권유가 아닙니다. 레버리지 ETF는 극단적 리스크를 내포하며, 원금 손실 가능성이 매우 높습니다. 투자 결정은 투자자 본인의 책임이며, 레버리지 메커니즘 및 변동성 감쇠를 충분히 이해한 후 투자해야 합니다. 과거 성과는 미래 수익을 보장하지 않습니다. 세무 및 법률 자문은 전문가와 상담하시기 바랍니다.
