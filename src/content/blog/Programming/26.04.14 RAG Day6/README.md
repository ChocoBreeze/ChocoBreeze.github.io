...
## 핵심 개념
- **DocumentLoader → TextSplitter → VectorStore → Retriever → Chain**
- **RetrievalQA**: 검색과 답변 생성을 하나의 체인으로 묶는 방식
- **Conversational RAG**: 이전 대화를 기억하며 검색에 반영하는 방식
- **다중 문서 처리**: 여러 파일을 한 번에 로드해 하나의 지식 베이스로 통합하는 방식
...
**반은 잘 나온 거고, 반은 아직 확인이 더 필요한 상태**예요.

정확히 말하면:

<h2> 잘 나온 부분 </h2>

이 출력만 보면 **파이프라인 자체는 정상 동작**했어요.

* `로드된 문서 수: 16`
  → 문서 로딩이 됐다는 뜻이에요.
  특히 PDF는 보통 **페이지 단위로 여러 Document**로 읽혀서, txt 1개 + PDF 여러 페이지 = 16개처럼 나오는 게 자연스러워요.
...
즉, **코드가 죽지 않고 RAG 흐름은 연결됐다**는 점에서는 성공입니다.
...
이렇게 나온 건, **질문에 맞는 근거를 문서에서 제대로 못 찾았을 가능성**이 커요.
...
**코드는 대체로 잘 됐고, 지금 문제는 “RAG 파이프라인 오류”라기보다 “질문-문서 매칭 문제”일 가능성이 큽니다.**

<h2> 바로 확인해보면 좋은 것 </h2>

먼저 **문서에 실제로 있는 질문**으로 바꿔보세요.
...
이렇게 **문서 내용과 가까운 질문**을 했을 때 답변이 잘 나오면,
그건 파이프라인이 제대로 된 거예요.

<h2> 하나 더 해보면 좋은 점검 </h2>

지금은 출처 파일명만 나오니까,
**실제로 어떤 청크가 검색됐는지 내용도 같이 출력**해보면 좋아요.
...
네, 지금 로그를 보면 **파이프라인은 정상**이고, 사용자 입장에서 느낀 그 이상함도 **정확히 짚으신 게 맞아요**.
...
* 상세 내용도 실제로 **“Berkshire’s Performance vs. the S&P 500”** 표를 가져옴
* 답변도 그 표를 바탕으로 요약하고 있음

즉, 이 경우는 **질문 → 적절한 청크 검색 → 답변 생성**이 잘 이어졌습니다.

다만 밑에 같은 페이지가 여러 번 나온 건 조금 아쉬워요.
이건 보통 **같은 페이지에서 잘린 여러 청크가 비슷하게 검색됐기 때문**이에요. 완전히 고장난 건 아니고, **검색 결과가 중복된 상태**에 가깝습니다.

<h2> 2) “버크셔 해서웨이의 2024년 수익률은 몇 퍼센트인가요?” 결과 </h2>

이건 **파이프라인 실패라기보다, 검색이 질문을 약간 빗나간 경우**로 보는 게 맞아요.

왜 그렇게 보이냐면:

* 사용자는 **“2024년 수익률(annual return, percentage change)”** 을 물었어요.
* 그런데 검색된 대표 청크는 `2024ltr.pdf p.4`의 **2024 earnings** 표예요.
* 즉, retriever가 **“2024”** 라는 숫자에는 잘 반응했는데,
  사용자가 원하는 **“주가/시장가치 기준 수익률 표”** 가 아니라
  **실적(earnings)** 쪽 청크를 끌고 온 거예요.
...
즉, **2024 행은 아마 표의 아래쪽**에 있는데,
현재 검색된 청크는 **표 제목 + 위쪽 연도들** 쪽만 담고 있는 가능성이 큽니다.

그래서 retriever는

* “Berkshire’s Performance vs. the S&P 500”라는 제목 때문에 p.14를 찾긴 했지만,
* 정작 **2024 행이 들어 있는 청크**를 못 잡았을 수 있어요.

이건 **표가 있는 PDF에서 chunk_size가 너무 작을 때 자주 생기는 현상**이에요.
...
즉, 의미는 통하지만,
retriever 입장에서는 **“2024”와 연결되는 다른 청크들(p.4의 earnings, txt의 2024 투자 메모)** 도 꽤 강한 후보가 될 수 있어요.

<h2> 그래서 지금 상태를 한 줄로 평가하면 </h2>

**S&P 500 비교 질문은 성공, 2024 수익률 질문은 검색 정밀도가 부족한 상태**예요.

즉,

* **RAG 자체는 잘 돌아감**
* **질문에 따라 검색 품질 차이가 나타남**
* **특히 표/숫자 질의에서 chunking 문제가 드러남**
...
이렇게 키워보면, p.14 아래쪽의 **2024 행까지 같은 청크 안에 들어올 가능성**이 커져요.
...
이렇게 하면 **비슷한 청크만 4개 뽑는 것보다**,
좀 더 **다양한 후보**를 섞어서 가져오려는 성향이 생깁니다.
...
이렇게 **문서 안의 실제 제목/표현을 섞어 주면** 검색이 훨씬 안정적이에요.
...
* **RAG는 “문서가 있으면 무조건 정답”이 아니다**
* **검색 단계가 빗나가면 답변도 흔들린다**
* **특히 표/숫자 질문은 chunking 방식이 중요하다**
* **같은 페이지 중복 검색은 retriever 설정으로 개선할 수 있다**
...
원하시면 제가 바로 다음 답변에서 **중복 검색 줄이기 + chunk_size 키우기**까지 반영한 수정 코드만 딱 뽑아서 드릴게요.
...
**MMR(Maximal Marginal Relevance)** 는 검색 결과를 뽑을 때
**“질문과 얼마나 관련 있는가”** 와 **“이미 뽑은 결과들과 너무 비슷하지는 않은가”** 를 같이 보려는 방식이에요. LangChain 문서에서도 MMR를 **질문과의 유사도와 결과들 사이의 다양성 사이를 균형 있게 맞추는 방법**으로 설명합니다.

지금처럼 일반 similarity search만 쓰면, 벡터 DB는 보통 **질문과 가장 비슷한 청크들**을 순서대로 가져와요. 그래서 같은 페이지에서 잘린 청크 여러 개가 서로 거의 비슷하면, 상위 결과가 전부 그 페이지 근처 청크로 채워질 수 있어요. LangChain 쪽 설명도 similarity search와 별도로 MMR를 제공하면서, MMR가 **중복되거나 너무 비슷한 결과를 줄이는 데 유용하다**고 안내합니다.
...
p.14 안에 표 제목과 연도 표가 여러 청크로 잘렸고, 그 청크들이 서로 아주 비슷하니까, 일반 검색은 **“이것도 관련 높음, 이것도 관련 높음, 이것도 관련 높음”** 하면서 거의 같은 결과를 여러 개 뽑은 거예요. MMR는 이때 **첫 번째로 좋은 청크를 뽑은 뒤**, 그다음 후보를 고를 때는 **질문과의 관련성은 유지하되 이미 고른 청크와 너무 비슷한 후보는 덜 선호**하게 됩니다.
...
* `k=4`
  → 그중에서 **관련성 + 다양성**을 같이 고려해서 최종 4개 선택하기
...
* 하나의 문서뿐 아니라 여러 문서에서 **조금씩 다른 근거**를 가져오고 싶을 때

다만 MMR가 만능은 아니에요.
중복은 줄여줄 수 있지만, **아예 질문을 잘못 이해한 검색**까지 자동으로 고쳐주지는 못해요. 예를 들어 “2024 수익률” 질문에서 p.4의 earnings 표가 강하게 잡히는 문제는, MMR만으로 해결된다기보다 **청크 크기**, **질문 표현**, **표가 잘 보존되도록 문서를 자르는 방식**도 함께 영향을 줍니다. 문서 청킹이 검색 품질에 중요하다는 점은 LangChain 문서에서도 반복해서 강조합니다.

딱 한 줄로 기억하면 이거예요.

> **MMR는 “관련성만 높은 결과 여러 개” 대신, “관련성도 높고 서로 덜 겹치는 결과들”을 뽑게 해주는 검색 방식**입니다.

원하시면 다음엔 제가 `k`, `fetch_k`, 그리고 가끔 보이는 `lambda_mult`가 각각 무슨 역할인지도 이어서 아주 쉽게 설명해드릴게요.

- [ZeusDB integrations - Docs by LangChain](https://docs.langchain.com/oss/python/integrations/providers/zeusdb?utm_source=chatgpt.com)
- [Vector store integrations - Docs by LangChain](https://docs.langchain.com/oss/python/integrations/vectorstores?utm_source=chatgpt.com)
- [OpenSearch integration - Docs by LangChain](https://docs.langchain.com/oss/python/integrations/vectorstores/opensearch?utm_source=chatgpt.com)
- [MemoryVectorStore integration - Docs by LangChain](https://docs.langchain.com/oss/javascript/integrations/vectorstores/memory?utm_source=chatgpt.com)
- [Docugami integration - Docs by LangChain](https://docs.langchain.com/oss/python/integrations/document_loaders/docugami?utm_source=chatgpt.com)
...
`k`는 **최종적으로 돌려줄 문서 개수**예요. LangChain의 `as_retriever(..., search_type="mmr")` 기준으로 `k`의 기본값은 4입니다.

`fetch_k`는 **MMR가 고르기 전에 먼저 넓게 가져오는 후보 개수**예요. 즉, “일단 몇 개를 후보로 모아둘까?”에 해당합니다. 기본값은 20입니다. 후보를 넓게 모아야 그 안에서 중복이 덜한 결과를 다시 고르기 쉬워집니다.

`lambda_mult`는 **관련성 vs 다양성의 균형값**이에요. 0과 1 사이 값을 쓰고, LangChain 문서 기준으로 `1`에 가까울수록 **다양성은 적고** 질문과 더 비슷한 결과를 선호하며, `0`에 가까울수록 **다양성은 크고** 서로 덜 비슷한 결과를 더 강하게 섞습니다. 기본값은 0.5입니다.
...
- [as_retriever | langchain_core](https://reference.langchain.com/python/langchain-core/vectorstores/base/VectorStore/as_retriever?utm_source=chatgpt.com)
- [ZeusDB integrations - Docs by LangChain](https://docs.langchain.com/oss/python/integrations/providers/zeusdb?utm_source=chatgpt.com)
...
네, 이 결과는 **이전보다 좋아졌지만 아직 완전히 맞지는 않은 상태**예요.
...
* `생성된 청크 수: 42`
  ...
  * 이전 91개에서 줄었죠.
  * 즉 **청크를 더 크게 잡은 효과**가 실제로 반영됐어요.
* 출처에 `2024ltr.pdf p.14`가 들어옴

  * 이건 아주 중요해요.
  * 이제 적어도 **버크셔 vs S&P 500 표가 있는 쪽**을 검색하기 시작했다는 뜻입니다.

즉, **retrieval 단계는 이전보다 나아졌어요.**
...
이건 지금 상태를 보면 **완전한 성공은 아니고, 검색은 가까워졌지만 답변 생성이 아직 헷갈린 상태**로 보는 게 맞아요.

특히 출처를 보면:

1. `2024ltr.pdf p.4` → **earnings 표**
2. `sample_investment_note.txt` → **완전히 다른 삼성전자 문서**
3. `2024ltr.pdf p.14`
4. `2024ltr.pdf p.14`
...
* 어떤 문서는 **2024 earnings**
* 어떤 문서는 **삼성전자 반도체 업황**
* 어떤 문서는 **버크셔 vs S&P 500 표**

이렇게 **서로 다른 문맥이 한 번에 들어간 상태**예요.
...
* **가장 관련 있는 청크만 깔끔하게 모이지 않음**
* **불필요한 문서(p.4, sample_investment_note.txt)가 같이 들어옴**
* 그래서 최종 답변이 흔들림

즉,

> **검색 품질은 개선됐지만, 아직 컨텍스트 노이즈가 남아 있다**
...
이건 **출력 길이를 500자로 잘라서 보여줬기 때문**일 가능성이 커요.
즉, 실제 청크 안에는 그 뒤에 **2017~2024**가 계속 있을 수 있어요.

그래서 지금 꼭 해야 하는 확인은:

**정말 p.14 청크 안에 2024 행이 들어 있는지 직접 보는 것**입니다.

이걸 확인해야 해요.

* 들어 있다 → 검색은 성공, **LLM 답변 단계 문제**
* 안 들어 있다 → 아직도 **청킹/검색 문제**
...
이렇게 하면 p.14 청크에서 **2024 행이 실제로 보이는지** 확인할 수 있어요.
...
그다음은 **retriever가 너무 많은 잡문서를 섞고 있는 문제**예요.
...
지금 결과는 **“실패”는 아니고, 꽤 유의미하게 개선된 중간 단계**예요.

특히 좋은 건:

* 이전에는 완전히 엉뚱한 문서 중심이었는데
* 지금은 **정답이 있을 법한 p.14까지는 도달했다**는 점입니다.

이건 꽤 큰 진전이에요.

<h2> 딱 한 줄로 정리 </h2>

**v2는 retrieval은 개선됐지만, 아직 irrelevant context 때문에 answer가 흔들리는 상태**입니다.

다음으로는 제가 추천한 대로
`doc.page_content[:2000]`로 바꿔서 **p.14에 2024 행이 실제로 있는지** 먼저 확인해보는 게 제일 좋습니다.

원하시면 그다음엔 제가
**“2024 행만 바로 찾는 디버깅용 출력 코드”** 를 딱 붙여드릴게요.
...
네, **어느 정도는 맞아요.**
실제로 RAG는 **원하는 답이 잘 나오도록 값들을 조정하는 과정**이 꼭 들어갑니다.

그런데 중요한 건,
**“질문 1개가 잘 될 때까지 감으로 튜닝하는 것”** 이 아니라
**“대표 질문 묶음 전체에서 안정적으로 잘 되게 조정하는 것”** 이라는 점이에요.
...
**“일단 돌아가는 기준선(baseline)”** 을 먼저 만드는 거예요.
...
이런 식으로 **대표 질문 20개, 50개, 100개**를 만들어 둡니다.
...
이런 식으로 바꿔보고,
평가용 질문 세트에 대해 **전체적으로 어떤 설정이 더 낫는지** 봅니다.

즉,
**“원하는 답이 나올 때까지 찍어보는 것”** 이 아니라
**“여러 설정 중 어떤 조합이 전체적으로 더 좋은지 비교 실험하는 것”** 에 가까워요.
...
즉, 프로젝트에서는
**“값만 조정”** 이 아니라
**문서 품질 + 검색 전략 + 프롬프트 + 평가 방식**을 같이 봐야 해요.
...
네, **실제로 원하는 답이 잘 나오도록 조정은 합니다.**
하지만 프로젝트에서는 **질문 하나를 맞추는 방식이 아니라, 대표 질문 세트 전체에서 가장 안정적으로 잘 작동하는 설정을 찾는 방식**으로 진행합니다.
...
네, **전반적으로는 잘 나온 편**이에요.  
다만 **완벽하게 깔끔한 상태는 아니고, “목표는 달성했지만 검색 노이즈가 조금 있다”** 정도로 보면 딱 맞습니다.
...
이렇게 나왔는데,
모델이 “그중”을 **첫 질문의 삼성전자 실적 요약**으로 이해하고 답했어요.

이건 바로 **Conversational RAG + memory가 작동했다**는 뜻입니다.
...
즉, **출처 조합이 달라졌기 때문에**, 질문에 따라 retriever가 다른 청크를 가져오긴 했어요.

다만 여기서 중요한 건:

* **달라지긴 했지만**
* 꼭 **더 좋아진 방향으로만 달라진 건 아니다**

라는 점이에요.

왜냐하면 삼성전자 질문인데 `2024ltr.pdf`가 계속 같이 끼어들고 있거든요.
이건 **검색이 완전히 정밀하지는 않다**는 뜻입니다.
...
즉, **맥락은 이해했지만, 답변 압축이 완전히 정확하지는 않았어요.**
...
* **대화형 맥락 이해는 됨**
* 하지만 **retrieval purity(검색 순도)** 는 아직 조금 낮음

이 상태예요.

<h2> 결과 평 </h2>

**“Conversational RAG의 핵심 동작은 확인되었고, 메모리도 작동했지만, 검색 결과에 불필요한 문서가 일부 섞여 답변이 완전히 정밀하지는 않다.”**
...
이건 **지금 코드가 당장 틀렸다는 뜻은 아니고**,
**최신 LangChain 기준으로는 예전 memory 방식이 deprecated(앞으로 권장되지 않음)** 라는 뜻이에요.

즉:

* **실행은 됨**
* **학습용 실습에는 문제 없음**
* 다만 **최신 방식으로는 나중에 바꾸는 게 좋음**
...
* **출처 유지:** 좋음
* **검색 정밀도:** 보통
* **질문 의도에 맞춘 답변 압축:** 보통~약간 아쉬움

즉, **Week 6 실습 통과 수준은 충분**해요.
