...
## 핵심 개념
- <strong>인덱싱</strong>: 문서를 청크로 나누고 임베딩해 벡터 DB에 저장하는 단계
- <strong>검색·생성</strong>: 질문에 맞는 청크를 찾고, 그 청크를 바탕으로 답변을 생성하는 단계
- <strong>시스템 프롬프트</strong>: 모델에게 답변 범위와 행동 원칙을 강제하는 지시문
- <strong>출처 추적</strong>: 답변과 함께 검색된 청크 출처를 보여 주는 방식
...
```
딱 한 가지 더 말씀드리면,  
지금 코드는 <strong>DB가 있으면 그냥 재사용</strong>하므로, 다른 파일로 바꿔 실행할 때는 `--rebuild`를 같이 주는 습관이 좋습니다.
...
네, 실습 결과 잘 나왔어요.  
그리고 이 출력은 <strong>Week 5에서 확인해야 할 포인트들을 꽤 잘 보여줍니다.</strong> 다만, <strong>딱 하나 중요한 관찰</strong>이 있어요: 지금은 <strong>기존 DB를 재사용하면서 sample txt와 PDF가 섞인 상태</strong>로 보입니다.
...
* <strong>벡터 DB를 사용하는 흐름은 정상</strong>
* 하지만 <strong>“이번 실행에서 문서가 먼저 인덱싱되었다”는 장면은 직접 확인하지 못함</strong>

즉, 이번 결과로 확인된 것은:

> <strong>인덱싱된 DB를 재사용해서 검색·생성이 잘 되는지</strong>
...
그러면 <strong>“문서가 먼저 인덱싱되는지”</strong> 를 눈으로 직접 확인한 것이 됩니다.
...
> <strong>벡터 DB가 질문과 유사한 청크를 찾아오는 검색 단계는 정상적으로 작동하고 있습니다.</strong>
...
* 그냥 답만 하는 게 아니라
* <strong>어떤 자료를 근거로 검색했는지 보여주고</strong>
* <strong>그걸 바탕으로 답변을 생성하고 있다</strong>
...
가 <strong>같이</strong> 나오고 있기 때문입니다.
...
실제로는 <strong>섞인 DB 전체를 대상으로 검색</strong>하고 있는 셈이에요.

그래서 실습 관점에서는 이렇게 해석해야 합니다.

* 검색 자체는 잘 됨
* 답변 제한도 어느 정도 잘 됨
* 하지만 <strong>실험 조건이 깔끔하게 분리되지는 않음</strong>
...
그 청크 안에 <strong>“최근 영업이익 추이”를 직접 설명하는 근거</strong>가 부족하면
모델이 억지로 답하지 않고 제한한 것이기 때문입니다.

즉, <strong>프롬프트가 어느 정도 잘 작동한 것</strong>으로 볼 수 있습니다.
...
완전히 “없다”로 끝내지 않고,
검색된 컨텍스트 안에서 <strong>관련성이 있는 부분만 제한적으로 설명</strong>했죠.
...
> <strong>컨텍스트 밖 질문에 대한 답변 제한</strong>

이 잘 확인된 것입니다.

<h2> 지금 상태에서 가장 추천하는 다음 실습 </h2>

이번엔 <strong>문서 하나만 대상으로 깔끔하게 다시 실험</strong>하는 게 좋아요.
...
좋아요. 이번 결과는 <strong>훨씬 깔끔하고, 오히려 Week 5 실습 목적에는 더 잘 맞는 결과</strong>예요.

이제는 DB가 섞이지 않았고, <strong>sample_investment_note.txt 하나만 대상으로 RAG가 동작한 것</strong>이 분명하게 보입니다.

<h2> 문서가 먼저 인덱싱되는지 </h2>

이번에는 이 항목이 <strong>명확하게 확인되었습니다.</strong>
...
즉, <strong>인덱싱 단계가 실제로 먼저 수행된 것</strong>을 확인한 겁니다.
...
이번에는 인덱싱 완료가 <strong>2개 청크 저장</strong>이었죠.
...
> <strong>검색 대상 문서가 너무 짧아서, 질문이 달라도 사실상 같은 두 청크 안에서만 찾는 상황</strong>
...
> <strong>“컨텍스트 밖 질문에는 답변을 제한한다”</strong>
...
> <strong>RAG가 문서 기반으로만 답하려고 시도했고, 근거가 부족하자 모른다고 답했다</strong>

는 점에서 좋은 결과입니다.

<h2> 이번 결과의 의미를 한 문장으로 정리하면 </h2>

> <strong>짧은 텍스트 문서를 대상으로 벡터 DB를 새로 구축한 뒤, 각 질문에 대해 관련 청크를 검색하고, 문서 내 직접적인 근거가 없을 경우 답변을 제한하는 기본 RAG 흐름을 확인하였다.</strong>
...
즉, <strong>실험 조건이 훨씬 깔끔해졌습니다.</strong>
...
네, 이 결과는 <strong>오류라기보다 질문이 문서와 안 맞아서 생긴 자연스러운 결과</strong>예요.

먼저 핵심부터 말씀드리면, `2024ltr.pdf`는 <strong>버크셔 해서웨이 2024년 주주서한</strong>이라서, 질문이 문서 주제와 맞아야 답이 잘 나옵니다. 실제 문서에는 2024년 실적, GEICO와 보험, Treasury Bill 투자, 일본 상사 투자, 현금/주식 배분, 세금, S&P 500 대비 성과 같은 내용이 중심이고, 삼성전자 영업이익 추이나 HBM 시장 전망은 주제가 아닙니다. 그래서 검색은 되었지만 답변은 “제공된 자료에서 찾을 수 없습니다”로 제한된 거예요. 
...
* <strong>인덱싱 성공</strong>
* <strong>검색 성공</strong>
* <strong>출처 출력 성공</strong>
* <strong>문서와 맞지 않는 질문에 대한 답변 제한도 성공</strong>

그러니까 “아무것도 안 됐다”가 아니라,
<strong>RAG가 문서 범위를 벗어난 질문을 억지로 답하지 않도록 잘 막아준 결과</strong>에 더 가깝습니다. 
...
이 문서는 <strong>버크셔 주주서한</strong>이지, 반도체 산업 보고서나 일반 챗봇용 문서가 아니기 때문입니다. 
...
좋아요. 이번 결과는 <strong>훨씬 잘 나왔고, RAG가 의도대로 작동한 사례</strong>예요.  
질문도 문서와 잘 맞았고, 답변도 전반적으로 맞습니다. 

먼저 한 줄로 요약하면:

> 이번에는 <strong>문서 주제에 맞는 질문을 넣었기 때문에</strong>, 검색된 청크를 바탕으로 실제 답변이 생성되었습니다. 즉, 앞선 “찾을 수 없습니다” 결과와 달리, 이번에는 <strong>검색도 적절했고 생성도 성공</strong>했습니다. 
...
즉, 지난번에는 “RAG가 안 된다”가 아니라 <strong>질문이 문서와 안 맞아서 제한 답변이 나온 것</strong>이었고, 이번에는 그 반대가 확인된 거예요. 

<h2> 질문별 분석 </h2>

<h3> 첫 번째 질문 </h3>

<strong>질문:</strong> 버크셔 해서웨이의 2024년 operating earnings는 얼마인가요?
<strong>답변:</strong> `$47,437 million`

이 답변은 <strong>정확합니다.</strong>
...
* 문장형 표현: 약 <strong>$47.4 billion</strong>
* 표 기준 표현: <strong>$47,437 million</strong>
...
<strong>질문:</strong> 2024년에 버크셔 실적이 예상보다 좋았던 이유는 무엇인가요?
<strong>답변:</strong> 189개 사업 중 53%가 감소했지만, 예측 가능한 대규모 투자 수익 증가가 있었고, 국채 수익률 개선과 보유 확대 때문이라고 설명함

이 답변도 <strong>대체로 맞습니다.</strong>
...
다만 지금 답변은 <strong>조금 아쉬운 점</strong>이 하나 있어요.
“보유 자산을 대폭 늘렸기 때문”이라고 했는데, 더 정확하게는 <strong>미국 단기 국채(Treasury Bills) 같은 단기 유동성 자산 보유를 크게 늘렸기 때문</strong>이라고 쓰는 게 문서에 더 충실합니다. 그리고 보험 사업, 특히 GEICO 개선도 함께 넣어주면 더 완성도 높아집니다. 
...
<strong>질문:</strong> 버크셔가 투자한 일본 5대 상사는 어디인가요?
<strong>답변:</strong> ITOCHU, Marubeni, Mitsubishi, Mitsui, Sumitomo

이 답변도 <strong>정확합니다.</strong>
...
즉, RAG에서는 <strong>“질문 품질”이 매우 중요하다</strong>는 점이 확실히 드러났습니다. 
...
* <strong>검색은 완벽하지 않을 수 있어도</strong>
* <strong>핵심 청크만 하나라도 잘 잡히면 생성이 잘 될 수 있다</strong>
  는 것도 보여줍니다.
...
네, 이번 결과도 전체적으로 보면 <strong>검색은 잘 되고 있고</strong>, 답변은 <strong>일부는 정확</strong>, <strong>일부는 문서에 있는데도 놓친 경우</strong>가 섞여 있어요.  
즉, 지금 단계에서는 <strong>기본 RAG는 성공</strong>했고, 동시에 <strong>검색·프롬프트 한계도 같이 드러난 결과</strong>입니다. 
...
* <strong>문서에 답이 있는데도 “찾을 수 없습니다”가 나온 질문</strong>: 2개

  * IRS에 납부한 법인세
  * 현금보다 equities를 선호하는 이유
  * 그리고 S&P 500 비교 질문도 사실상 문서에 답이 있는데 제한됨

정확히 말하면 <strong>3개가 아쉬운 결과</strong>예요.
이건 문서에 답이 없어서라기보다, <strong>질문 표현과 검색된 청크 조합, 그리고 현재 프롬프트의 엄격함</strong> 때문에 생긴 현상으로 보는 게 맞습니다. 

<h2> 1. 2024년 버크셔의 insurance-investment income은 얼마인가요? </h2>

<strong>답변:</strong> 13,670 백만 달러

이건 <strong>정확합니다.</strong>
...
<strong>답변:</strong> Todd Combs가 5년 동안 GEICO를 재편성했고, 효율성 개선과 underwriting practices 현대화가 있었으며 2024년 개선이 두드러졌다고 설명

이것도 <strong>정확합니다.</strong>
...
다만 영어 표현 “underwriting practices”를 한국어로 옮길 때 지금 답변은 “인수 관행”이라고 썼는데, 문맥상 <strong>보험 인수 심사/언더라이팅 관행</strong> 정도로 이해하면 더 자연스럽습니다. 
...
<strong>답변:</strong> 제공된 자료에서 찾을 수 없습니다.

이건 <strong>아쉽지만 오답</strong>입니다.
문서에 분명히 나와 있습니다. 버크셔는 2024년에 IRS에 총 <strong>$26.8 billion</strong>을 납부했다고 적고 있습니다. 문장도 매우 직접적입니다. “Berkshire last year made four payments to the IRS that totaled $26.8 billion.”이라고 되어 있어요. 

<h3> 왜 이런 일이 생겼을까요? </h3>

검색 결과가 p.5만 잡혔습니다. 그런데 IRS 세금 이야기는 p.5 하단에서 p.6 쪽으로 넘어갑니다. 현재 청크 분할이 작고, 검색이 p.5 중심으로만 잡히면서 <strong>정작 세금 숫자가 적힌 청크를 못 가져온 가능성</strong>이 큽니다. 
...
즉, “법인세”라는 한국어 표현보다 문서 표현인 <strong>IRS</strong>, <strong>payments</strong>, <strong>corporate income tax</strong> 쪽으로 맞추면 검색이 더 잘 붙을 수 있습니다. 

<h3> 정답 </h3>

* <strong>$26.8 billion</strong> 

<h2> 4. 버크셔는 왜 현금보다 equities를 선호한다고 말하나요? </h2>

<strong>답변:</strong> 제공된 자료에서 찾을 수 없습니다.

이것도 <strong>문서에 답이 있는데 놓친 경우</strong>입니다.
...
검색 결과는 p.7, p.7, p.6으로 나와서 사실 페이지 자체는 꽤 맞습니다. 그런데 질문이 <strong>“왜 선호하나요?”</strong> 처럼 설명형이라, 검색된 청크에 이유가 흩어져 있었거나 문장 연결이 충분히 안 되었을 수 있습니다. 또 프롬프트가 엄격해서 직접적인 근거가 부족하다고 판단했을 수도 있습니다. 
...
<strong>답변:</strong> $46 billion에서 $171 billion
...
이건 <strong>문서에 답이 있는데 놓친 경우</strong>입니다.
표에 따르면 2024년 버크셔의 annual percentage change는 <strong>25.5%</strong>, S&P 500은 <strong>25.0%</strong> 입니다. 즉, 2024년에는 버크셔가 S&P 500보다 <strong>약간 높았습니다.</strong> 
...
* 버크셔: <strong>25.5%</strong>
* S&P 500: <strong>25.0%</strong>
* 따라서 버크셔가 소폭 높음 
...
첫째, <strong>숫자가 문장이나 표에 직접 있는 질문</strong>은 잘 맞습니다.
insurance-investment income, float 같은 질문이 대표적입니다. 

둘째, <strong>설명형 질문이나 비교형 질문</strong>은 검색이 조금만 어긋나도 제한 답변이 나올 수 있습니다.
IRS 세금, equities 선호 이유, S&P 500 비교가 그렇습니다. 

셋째, 문서에 답이 있어도 <strong>질문 표현을 문서 표현과 더 가깝게 맞추면 성능이 좋아집니다.</strong>
예를 들어:
...
즉, <strong>질문을 외워서 미리 넣는 작업</strong>이 아니라  
<strong>시스템을 점검하고 다듬는 작업</strong>이라고 보시면 됩니다.
...
네, 방향은 맞는데 지금 상태는 <strong>조금 너무 짧고</strong>, 특히 두 번째 답은 더 정확하게 다듬는 게 좋아요.

제가 보면 이렇게 정리하면 훨씬 좋습니다.

<h2> 실습 후 직접 답해보기 </h2>

* <strong>RAG에서 검색이 먼저이고 생성이 나중인 이유는 무엇일까?</strong>
...
* <strong>컨텍스트를 주지 않고 바로 질문하면 어떤 문제가 생길까?</strong>
...
* <strong>왜 “모르면 모른다고 답하라”는 시스템 프롬프트가 중요한가?</strong>
...
* <strong>“검색한 데이터를 사용해서 생성을 해야 기존 LLM이 학습한 내용에 없는 내용에 대해 답변이 가능함.”</strong>
  → 맞는 말인데, 여기에 <strong>문서 기반 답변</strong>이라는 표현을 넣으면 더 좋아요.

* <strong>“컨텍스트라 하면 구체적으로 무엇을 말하는지?”</strong>
  → 이건 꼭 정리하고 넘어가는 게 좋아요.
  여기서 컨텍스트는 보통 <strong>검색된 청크 묶음</strong>입니다.

* <strong>“엉뚱한 정보를 주는 것보다는 그게 낫기 때문에.”</strong>
  → 틀리진 않지만, 보고서/과제용으로는
  <strong>환각 방지</strong>, <strong>문서 근거 유지</strong>
  이런 표현이 들어가면 더 좋아요.
...
좋아요. 이런 추천 자료들은 <strong>전부 처음부터 정독해야 하는 자료</strong>라기보다, <strong>목적별로 골라서 보는 참고자료</strong>라고 생각하시면 훨씬 편해요.

지금 Week 5 기준에서는 이렇게 보면 좋습니다.

<h2> 1. RAG from Scratch — LearnByBuilding.ai </h2>

이 자료는 제목 그대로 <strong>“처음부터 RAG를 직접 만드는 흐름”</strong> 을 보는 데 좋아요.
...
즉, <strong>Week 5 복습용으로 가장 직접적인 자료</strong>입니다.

<h2> 2. SingleStore RAG Tutorial </h2>

이건 보통 <strong>조금 더 서비스/구조 관점</strong>에서 보는 자료에 가까워요.
...
<strong>“RAG를 왜 이렇게 구성하는가”</strong> 를 이해하는 자료로 보면 좋아요.
...
즉, 이건 <strong>개념 확장용</strong>으로 보면 됩니다.

<h2> 3. Prompt Engineering Guide </h2>

이건 RAG 전용 자료라기보다,
<strong>프롬프트를 어떻게 써야 모델 행동을 제어할 수 있는가</strong>를 보는 자료예요.
...
전체를 다 볼 필요는 없고,
<strong>시스템 프롬프트 / 제약 조건 / 지시문 설계</strong> 관련 부분만 먼저 보시면 됩니다.
...
즉, 이건 <strong>검색 이후 생성 단계를 더 잘 다루기 위한 자료</strong>예요.
...
<strong>RAG from Scratch — LearnByBuilding.ai</strong>
...
<strong>SingleStore RAG Tutorial</strong>
...
즉, <strong>책처럼 정독</strong>하기보다
<strong>필요할 때 찾아보는 사전처럼</strong> 쓰는 게 훨씬 좋습니다.

<h2> Week 5 기준으로 각 자료에서 얻어가면 좋은 한 가지씩 </h2>

* <strong>RAG from Scratch</strong>
  → RAG 파이프라인 전체 흐름
...
출처는 <strong>“이 답이 어디서 나왔는지 확인하게 해주는 근거”</strong> 라서 필요해요.

RAG에서는 모델이 그냥 말만 하는 것이 아니라, <strong>검색된 문서 내용을 바탕으로 답해야</strong> 하잖아요. 그런데 출처가 없으면 사용자는 이런 걸 확인할 수 없습니다.

* 이 답이 진짜 문서에 있는 내용인지
* 모델이 지어낸 건 아닌지
* 문서의 어느 부분을 근거로 말한 건지

즉, 출처가 있어야 <strong>답변의 신뢰성</strong>을 확인할 수 있어요.

조금 더 나누어 보면:

* <strong>검증 가능성</strong>
...
<strong>답변과 함께 출처가 필요한 이유</strong>
...
<strong>답변과 함께 출처가 필요한 이유는, 답변의 근거를 검증하고 문서 기반 답변인지 확인하기 위해서이다. 또한 모델의 환각 여부를 점검하고, 문제가 검색 단계인지 생성 단계인지 파악하는 데도 도움이 된다.</strong>
