...
이 문구는 **파일이 수정되었지만 아직 커밋 준비 영역에 올라가지 않았다는 뜻**입니다.
...
이 문구는 **커밋에 포함될 준비가 된 변경 사항**을 의미합니다.
...
이 문구는 **Git이 아직 추적하지 않는 새 파일**이 있다는 뜻입니다.
...
즉, `git status -sb`는 **브랜치 정보를 포함해서 상태를 짧은 형식으로 보여주는 명령어**입니다.
...
이 의미는 현재 로컬 `main` 브랜치가 원격의 `origin/main`보다 **2커밋 뒤처져 있다**는 뜻입니다.
