...
## 핵심 개념
- <strong>메타데이터 필터링</strong>: 특정 연도, 특정 파일, 특정 카테고리만 검색하는 방식
- <strong>BM25</strong>: 키워드 일치에 강한 전통적인 검색 방식
- <strong>벡터 검색</strong>: 의미적 유사성을 보는 검색 방식
- <strong>하이브리드 검색</strong>: BM25와 벡터 검색을 결합한 방식
- <strong>MMR</strong>: 비슷한 결과만 반복되지 않도록 다양성을 고려하는 검색 방식
...
네, 결과는 <strong>전체적으로 정상적으로 실행된 것이 맞습니다.</strong>
...
이건 <strong>아주 정상적인 결과</strong>입니다.
...
즉, 이 결과는 <strong>필터가 잘못 작동한 게 아니라, 필터가 너무 강하게 적용된 것</strong>입니다.

<h2> 3. 이 결과로 배울 수 있는 핵심 </h2>

이번 결과는 Week 7 학습에 오히려 아주 좋습니다.

메타데이터 필터링은 검색 범위를 줄여주는 강력한 기능이지만, <strong>메타데이터를 잘못 설계하면 좋은 문서를 검색 대상에서 제외할 수 있습니다.</strong>
...
일반 벡터 검색은 비슷한 청크를 계속 가져올 수 있는데, MMR은 <strong>관련성 + 다양성</strong>을 같이 봅니다.
...
이번 결과는 <strong>성공적으로 실행된 결과</strong>입니다.
다만 동시에 이런 교훈을 보여줍니다.
...
네, 이 결과들은 <strong>“정답을 얻었다”기보다, 검색 방식별 성격과 문제점을 진단하는 결과</strong>로 보면 됩니다.  
결론부터 말하면, 실행은 잘 됐고 Week 7 학습 목적에도 맞습니다. 다만 지금 결과에서는 <strong>중복 저장</strong>, <strong>메타데이터 설계 문제</strong>, <strong>질문-문서 불일치</strong>가 같이 드러났어요. 
...
즉, 지금 결과는 실패가 아니라 <strong>검색 시스템을 개선할 포인트가 잘 보인 결과</strong>입니다.
...
즉, 이 결과는 <strong>BM25가 약하다 / 벡터 검색이 약하다</strong>라기보다,
...
이 질문은 <strong>의미 기반 질문</strong>입니다.
...
즉, MMR은 <strong>비슷한 결과 반복을 줄이고 다양한 청크를 보여주는 효과</strong>가 있습니다.
...
벡터 검색은 <strong>질문과 의미가 비슷한 청크</strong>를 찾습니다.
...
BM25는 <strong>정확한 키워드가 있는 청크</strong>를 잘 찾습니다.
...
MMR은 <strong>비슷한 결과 반복을 줄이고 다양성을 확보</strong>하는 검색입니다.
...
> <strong>중복 저장 문제는 어느 정도 해결됐고, 지금 남아 있는 문제는 “문서 풀 자체에 관련 문서가 너무 적다” + “메타데이터 필터 설계가 질문과 안 맞는다”입니다.</strong>
...
그래서 지금 결과는 검색기가 틀렸다기보다, <strong>문서 풀에 관련 청크가 부족한 상태에서 k=3을 요구했기 때문에 생긴 결과</strong>입니다.
...
즉, 필터가 잘못 작동한 게 아니라 <strong>필터 조건 때문에 실제 정답 문서를 제외한 것</strong>입니다.
...
MMR은 중복을 줄여주지만, <strong>없는 관련 문서를 만들어주지는 못합니다.</strong>
...
이번 결과는 <strong>정상이고, 오히려 이전보다 훨씬 깨끗해졌습니다.</strong>
...
이 코드는 <strong>chunks를 Chroma DB에 저장</strong>합니다.
...
이게 제가 말한 <strong>DB 중복 저장 문제</strong>입니다.

<h2> 그럼 Week7 코드는 매번 삭제해야 하나요? </h2>

학습 실습 기준으로는 <strong>네, 지금 Week7 코드 구조에서는 매번 삭제하고 실행하는 편이 가장 깔끔합니다.</strong>
...
---
네, 방향은 맞는데 <strong>답안으로 쓰기에는 조금 짧고 추상적</strong>이에요. Week 7의 핵심을 보여주려면 “BM25 / 벡터 검색 / 필터 / 하이브리드”가 각각 왜 필요한지 한두 문장씩 들어가면 더 좋습니다.
