...
- **Streamlit**: Python 코드만으로 간단한 웹 앱을 만들 수 있는 라이브러리
- **파일 업로드 UI**: 사용자가 PDF 또는 TXT 파일을 화면에서 업로드하는 기능
- **URL 입력 UI**: 웹 기사 URL을 입력해 지식 베이스에 추가하는 기능
- **질문 입력 UI**: 사용자의 질문을 입력받고 RAG 답변을 출력하는 기능
- **카테고리 선택**: 특정 카테고리 자료만 검색하도록 범위를 제한하는 기능
- **참고 자료 출력**: 답변 생성에 사용된 원본 문서 정보를 함께 보여주는 기능
