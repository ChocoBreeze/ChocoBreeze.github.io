...
1. <strong>모든 가능한 부분 문자열 추출 (중첩 루프)</strong>:  
   ```cpp
   for (int start = 0; start < s.length(); start++) {
       string currString;
       for (int end = start; end < s.length(); end++) {
...
1. <strong>부분 문자열 추출 방식 변경</strong>  
