...
<strong>전략: "항상 분모(총원)가 가장 작은 반에 추가"를 따른 경우</strong>
...
<strong>대조: 두 번째 학생은 분모가 더 크지만 클래스 A에 배치</strong>
...
#### GPT
이 코드는 LeetCode "Maximum Average Pass Ratio" 문제를 해결하기 위한 <strong>우선순위 큐(힙)</strong> 기반의 효율적인 접근 방식을 보여줍니다. 
...
3. <strong>extraStudents 분배</strong>:
   ```cpp
   while (extraStudents--) {
       auto [currentGain, classInfo] = maxHeap.top();
       maxHeap.pop();
