...
### <strong>코드 분석</strong>
1. <strong>입력 및 변수 선언</strong>
   - 문자열 `s`가 주어짐.
   - `ans` 변수를 선언하여 최대 점수를 저장함.

2. <strong>문자열을 나누는 기준 설정</strong>
...
총 시간 복잡도: <strong>$O(n^2)$</strong> (비효율적)
...
## <strong>🔍 코드 분석</strong>
### <strong>1️⃣ 전체 '1' 개수 세기</strong>
```cpp
int ones = count(s.begin(), s.end(), '1');
```
- 문자열 `s` 전체에서 `'1'`의 개수를 세어 `ones` 변수에 저장합니다.  
- 이렇게 하면, 나중에 왼쪽 부분이 확장될 때 <strong>오른쪽 부분의 '1' 개수를 빠르게 업데이트</strong>할 수 있습니다.  
- `count()` 함수의 시간 복잡도는 `O(n)`입니다.

---

### <strong>2️⃣ 왼쪽에서 한 글자씩 이동하면서 최댓값 찾기</strong>
```cpp
int ans = 0;
int zeros = 0;
for (int i = 0; i < s.size() - 1; i++) {
...
- `i`를 `0`부터 `s.size() - 2`까지 반복하여 <strong>왼쪽 부분을 확장</strong>해 나감.

---

### <strong>3️⃣ 왼쪽 부분의 '0' 개수 & 오른쪽 부분의 '1' 개수 관리</strong>
```cpp
if (s[i] == '1') {
...
### <strong>4️⃣ 현재 분할 상태의 점수 갱신</strong>
```cpp
ans = max(ans, zeros + ones);
```
...
|:---:|:---:|:---:|:---:|:---:|:---:|
| 0    | `"0"`   | `"11101"`  | 1       | 3      | <strong>4</strong>         |
| 1    | `"01"`  | `"1101"`   | 1       | 2      | <strong>3</strong>         |
| 2    | `"011"` | `"101"`    | 1       | 1      | <strong>2</strong>         |
| 3    | `"0111"`| `"01"`     | 1       | 1      | <strong>2</strong>         |
| 4    | `"01110"`| `"1"`     | 2       | 1      | <strong>3</strong>         |

최댓값은 <strong>4</strong>.
...
2. <strong>왼쪽 부분이 확장될 때 오른쪽 부분의 '1' 개수를 효율적으로 업데이트</strong> → $O(n)$
3. **한 번의 `for` 루프로 문제 해결** → $O(n)$

---

## <strong>✅ 결론</strong>
이 코드는 <strong>최적화된 $O(n)$ 풀이법</strong>을 사용하여 문제를 해결합니다.  
...
#### GPT
이 코드는 문자열 `s`를 두 부분으로 나누어 왼쪽 부분에서 `'0'`의 개수를 최대화하고, 오른쪽 부분에서 `'1'`의 개수를 최대화하는 문제를 해결하는 <strong>효율적인 방법</strong>을 사용합니다.  
...
### <strong>2️⃣ 문자열을 순회하며 왼쪽 부분 점수 계산</strong>
```cpp
for (int i = 0; i < s.size() - 1; i++) {
```
...
### <strong>3️⃣ 현재까지의 '0'과 '1' 개수 세기</strong>
```cpp
if (s[i] == '1') {
...
### <strong>4️⃣ 최적 점수 갱신</strong>
```cpp
best = max(best, zeros - ones);
```
- `zeros - ones`를 사용하여 최적 점수를 계산  
- <strong>왼쪽 부분에서 '0'을 최대화하고 '1'을 최소화하는 것이 유리</strong>  
...
### <strong>5️⃣ 마지막 문자가 '1'인지 확인하여 추가</strong>
```cpp
if (s[s.size() - 1] == '1') {
    ones++;
}
...
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| 0    | `"0"`   | `"11101"`  | 1       | 0      | `1 - 0 = 1`   | <strong>1</strong>  |  
| 1    | `"01"`  | `"1101"`   | 1       | 1      | `1 - 1 = 0`   | <strong>1</strong>  |  
| 2    | `"011"` | `"101"`    | 1       | 2      | `1 - 2 = -1`  | <strong>1</strong>  |  
| 3    | `"0111"`| `"01"`     | 1       | 3      | `1 - 3 = -2`  | <strong>1</strong>  |  
| 4    | `"01110"`| `"1"`     | 2       | 3      | `2 - 3 = -1`  | <strong>1</strong>  |  
...
3. <strong>O(n) 단일 루프로 해결 가능하여 효율적</strong>
